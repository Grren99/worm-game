...
- 📣 **실시간 이벤트 피드**: 전용 컴포넌트가 킬·골든·파워업 피드를 관리하여 필터/사운드/펄스 연출을 안정적으로 동기화
- 🎚 **이벤트 피드 컨트롤**: 카테고리별 표시 여부와 효과음 볼륨을 사용자 설정 패널에서 취향대로 조절
...
- 이벤트 피드 필터/하이라이트/펄스 로직은 `EventFeedView`(`public/js/core/eventFeed.js`)로 분리돼 UI 업데이트와 사용자 설정 저장을 전담합니다
